package com.basya.weatherapp.ui
...
import com.basya.weatherapp.BuildConfig
import com.basya.weatherapp.R
...
import com.basya.weatherapp.databinding.ActivityMainBinding
import com.basya.weatherapp.utils.HelperFunction.formatDegree
import com.basya.weatherapp.utils.LOCATION_PERMISSION_REQ_CODE
import com.basya.weatherapp.utils.iconSizeWeather4x
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        searchCity()
        getWeatherByCity()
        getWeathetByCurrentLocation()
    }

    private fun getWeathetByCurrentLocation() {
        isLoading = true
        loadingStateView()
    ...
    private fun setUpView(weather: WeatherResponse?, forecast: ForecastWeatherResponse?) {
        weather?.let {
            binding.apply {
                ...
                setBackgroundPage(it.weather?.get(0)?.id, icon)
            }
        }
        forecast?.list.let {
    ...
    private fun setBackgroundPage(id: Int?, icon: String?) {
        id?.let {
            when (id) {
